  ...
  const handleFileDrop = useCallback((e: React.DragEvent) => {
    ...
    if (type === DragType.FILE || type === DragType.BOTH) {
      ...
      if (!fileProps.multiple && files.length > 1) {
        console.warn('只能上传一个文件', 'warning', '文件限制')
        return
      }

      if (fileProps.maxFiles && files.length > fileProps.maxFiles) {
        console.warn(`最多只能上传 ${fileProps.maxFiles} 个文件`, 'warning', '文件限制')
        return
      }
      ...
      try {
        ...
        console.warn(error instanceof Error ? error.message : '文件验证失败', 'error', '验证失败')
      }
    }
